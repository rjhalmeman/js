...
//window.onload = alert("algo");
window.onload = dadosInicias();

function dadosInicias() {
    listaJoias.push(new Joias(0, "Anel de Ouro", 'Ouro', '2023-01-15', 'Anel', '10'));
    listaJoias.push(new Joias(1, "Colar de Prata", 'Prata', '2022-11-20', 'Colar', '20'));
    listaJoias.push(new Joias(2, "Bracelete de Platina", 'Platina', '2021-05-30', 'Bracelete', '15'));
    listaJoias.push(new Joias(3, "Brinco de Ouro", 'Ouro', '2020-03-10', 'Brinco', '5'));
    listar();

}

function salvarNoComputador() {
    nomeParaSalvar = "./Joias.csv";
    let textoCSV = "";
...
function procure() {
    ...
    if (id) {
        joias = procurePorChavePrimaria(id); 
        if (joias) {
...
function salvar() {
    ...
    const metal = document.getElementById("inputMetal").value.trim();
    const dataFabricacao = document.getElementById("inputDataFabricacao").value.trim();
    const tipo = document.getElementById("inputTipo").value.trim();
    const peso = parseFloat(document.getElementById("inputPeso").value.trim()) || 0;

    if (id && nome && metal && dataFabricacao && tipo && peso > 0) {
        const novaJoia = new Joias (
            id,
            nome,
            metal,
            dataFabricacao,
        ...
        switch (oQueEstaFazendo) {
            case "inserindo":
                ...
                if (joias) {
                    joias.nome = nome;
                    joias.metal = metal;
                    joias.dataFabricacao = dataFabricacao;
                    joias.tipo = tipo;
                    joias.peso = peso;
...
function listar() {
    ...
    //deixei com um laço no padrão mais conhecido 
    for (let i = 0; i < listaJoias.length; i++) {
        const joia = listaJoias[i];
        html += "<tr>" +
            "<td>" + joia.id + "</td>" +
            "<td>" + joia.nome + "</td>" +
            "<td>" + joia.metal + "</td>" +
            "<td>" + joia.dataFabricacao + "</td>" +
            "<td>" + joia.tipo + "</td>" +
            "<td>" + (joia.peso || "0.00") + "</td>" +
...
function mostrarDadosJoias(joias) {
    ...
    document.getElementById("inputPeso").value = joias.peso;
    bloquearAtributos(true);
}
